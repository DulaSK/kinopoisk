...
const createFilm = async(req, res) => {
    ...
    if(
            req.file && req.body.titleRus.length > 2 &&
            ...
            if(req.body.video && req.body.video.length > 2){
                await new Film({
                    ...
                    image: `/img/films/${req.file.filename}`, //${req.file.destination}
                    author: req.user._id
                }).save()
...
const saveFilm = async (req, res) => {
    if(req.user && req.body.id){
        ...
        const findFilm = user.toWatch.filter(item => item.id == req.body.id)
...
const deleteFromToWatch = async (req, res) => {
    if(req.user && req.params.id){
        const user = await User.findById(req.user.id)
        const findFilm = user.toWatch.filter(item => item.id == req.params.id) 
